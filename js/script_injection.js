    ...
    function getStartupParameters(callback) {
        chrome.runtime.sendMessage({
            ...
            if (response == null) {
                ...
                callback(response.injectionId, response.isConnectedToProduct, response.tabId);
            }
        });
    }
    getStartupParameters(function (injectionId, isConnectedToProduct, currentTabId) {
        if (isConnectedToProduct) {
            plugin.callOnDocumentInteractive(function () {
                initApiInjection(currentTabId);
